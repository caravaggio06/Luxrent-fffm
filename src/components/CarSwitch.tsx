...
type Car = {
  ...
  dailyPrice: number; weekendPrice: number; deposit: number;
  powerHp: number; accel0to100: number; topspeedKmh: number;
  drivetrain: string; gearbox: string; consumptionL100: number;
  media: { poster: string; video: string; audio: string };
};

export default function CarSwitch({
  cars, activeIndex, onPrev, onNext, onStopAudio
}: { cars: Car[]; activeIndex: number; onPrev: () => void; onNext: () => void; onStopAudio: () => void }) {
  ...
  return (
    <section className="relative vh-100 isolate overflow-hidden">
      ...
      <div className="absolute inset-y-0 left-0 flex items-center">
        <button onClick={onPrev} aria-label="Vorheriges Fahrzeug"
          className="mx-2 md:mx-4 h-10 w-10 rounded-full bg-white/10 hover:bg-white/20">◀</button>
      </div>
      <div className="absolute inset-y-0 right-0 flex items-center">
        <button onClick={onNext} aria-label="Nächstes Fahrzeug"
          className="mx-2 md:mx-4 h-10 w-10 rounded-full bg-white/10 hover:bg-white/20">▶</button>
      </div>
        ...
        <div className="mb-6">
          ...
          <AnimatePresence mode="wait">
            <motion.h1
              ...
              animate={{ y: 0, opacity: 1, transition: { delay: .1, duration: .5 } }}
              exit={{ y: -12, opacity: 0, transition: { duration: .25 } }}
            >
              <span className="text-accent-weak">{car.brand}</span> {car.model}
            </motion.h1>
          </AnimatePresence>
          <div className="mt-1 text-xs text-zinc-300/80">{car.year} • {car.drivetrain} • {car.gearbox}</div>
        </div>
        ...
        <div className="pointer-events-auto absolute left-0 right-0 bottom-6 z-10">
          <div className="mx-auto max-w-7xl px-4 flex gap-3 overflow-x-auto">
            {cars.map((c: Car, idx: number) => (
              <button
                key={c.id}
                onClick={() => { onStopAudio(); idx !== activeIndex && onNext(); /* Trigger extern via Buttons falls gewünscht */ }}
                className={`px-3 py-2 rounded-xl border ${c.id===car.id?'border-accent text-white':'border-white/15 text-zinc-300'} whitespace-nowrap`}
              >
                {c.brand} {c.model}
              </button>
            ))}
